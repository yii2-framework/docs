...
Контролери складаються з _дій_, які є основними блоками, до яких може звертатись кінцевий користувач і запитувати
...
Кінцеві користувачі звертаються до дій з допомогою так названих _маршрутів_. Маршрут це текстовий рядок, який складається з наступних частин:
...
Створення дій може бути настільки ж простим, як і оголошення так званих _методів дій_ у класі контролера. Метод дії це
_публічний_ метод, ім’я якого починається зі слова `action`. Значення, яке повертається методом дії, представляє дані
...
> Note: Назви імен методів дій є _регістр-залежними_. Якщо у вас є метод `ActionIndex`, то його не буде враховано
...
перерозподілити дії, ви повинні визначити їх як _автономні дії_.
...
Для використання автономної дії, ви маєте вказати її у _мапі дій_ за допомогою перевизначення методу
...
_параметри дії_. Їх значення беруться із запитів. Для [[yii\web\Application|веб-додатків]], значення кожного з
