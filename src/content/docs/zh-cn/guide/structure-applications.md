...
该事件在应用处理请求 _before_ 之前，实际的事件名为 `beforeRequest`。
...
该事件在应用处理请求 _after_ 之后但在返回响应 _before_ 之前触发，
...
该事件在每个 [控制器动作](structure-controllers.md) 运行 _after_ 之后会被触发，
实际的事件名为 `afterAction`。
