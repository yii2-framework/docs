...
El cargador se instala cuando incluyes el archivo `vendor/autoload.php`.
...
`backend`. Esto permitirá que estas clases sean automaticamente cargadas por el autocargador de Composer.

## Mapa de Clases <span id="class-map"></span>

El autocargador de clases de Yii soporta el _mapa de clases_, que mapea nombres de clases to sus correpondientes rutas de
...
Puedes añadir una clase al mapa de clases Composer classmap de la siguiente forma,
...
archivo `vendor/autoload.php` _después_ de que todos los demás autocargadores se hayan instalado. Esto hará que el autocargador de
...
La primera línea instala el autocargador de Composer, mientras que la segunda línea instala el autocargador de Composer.

```php
require __DIR__ . '/../vendor/autoload.php';
```

Puedes usar el autocargador de Composer sin el autocargador de Composer. Sin embargo, al hacerlo, la eficacia de la carga de
...
> Note: Si no deseas utilizar el autocargador de Composer, tendrás que crear tu propia versión del archivo `vendor/autoload.php` e
...
El autocargador de Composer es capaz de autocargar clases de [extensiones](structure-extensions.md). El único requirimiento es
...
En el caso de que no quieras usar el autocargador de Composer, el autocargador de Composer podría cargar las clases de extensiones
por tí.
