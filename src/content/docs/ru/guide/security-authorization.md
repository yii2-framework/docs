...
Роль представляет собой набор разрешений (_permissions_) (например, создание сообщения, обновление сообщения).
...
С каждой ролью или разрешением может быть связано правило (_rule_). Правило представляет собой кусок кода, который будет
...
разрешения могут содержать другие разрешения. Yii реализует _частично упорядоченную_ иерархию, которая включает в себя
специальные _деревья_ иерархии. Роль может содержать разрешение, но обратное не верно.
...
![Простая иерархия RBAC](/docs/guide/images/rbac-hierarchy-1.png "Простая иерархия RBAC")
...
![Иерархия RBAC с правилом](/docs/guide/images/rbac-hierarchy-2.png "Иерархия RBAC с правилом")
...
![Проверка доступа](/docs/guide/images/rbac-access-check-1.png "Проверка доступа")
...
![Проверка доступа](/docs/guide/images/rbac-access-check-2.png "Проверка доступа")
...
![Проверка доступа](/docs/guide/images/rbac-access-check-3.png "Проверка доступа")
...
Роль по умолчанию — это роль, которая _неявно_ присваивается _всем_ пользователям. Вызов метода
