...
Active Record соответствует строке этой таблицы, а _атрибут_ объекта Active Record представляет собой значение
...
Т.к. класс [[yii\db\ActiveRecord]] наследует класс [[yii\base\Model]], он обладает _всеми_ возможностями
...
> Info: Т.к. класс [[yii\db\ActiveQuery]] наследует [[yii\db\Query]], вы можете использовать в нём _все_ методы
...
- [[yii\db\ActiveRecord::findAll()]]: возвращает массив объектов Active Record, заполненных _всеми_ полученными результатами запроса.
...
формата `'ДД.ММ.ГГГГ'`. Для достижения этой цели, вы можете объявить методы _преобразования данных_ в
...
В главе [Построитель запросов](db-query-builder.md) мы объясняли, что вы можете использовать _пакетную выборку_ для
...
_dirty-атрибуты_. Атрибут считается _dirty-атрибутом_, если его значение было изменено после чтения из базы данных или
...
Эта задача решается простым объявлением _методов получения связных данных_ для каждой интересующей вас связи как
показано ниже:
...
регистре) _именем связи_. Помните, что имена связей чувствительны к регистру.
...
связных данных. По этой причине, мы называем его _свойством связи_. Например:
...
только во время первого доступа к свойству связи. Мы называем подобный способ получения связных данных _отложенной
загрузкой_. Например:
...
Вы можете жадно загружать одну или несколько связей. Вы можете даже жадно загружать _вложенные связи_. Вложенная связь -
...
Вышеприведённый запрос вернёт _всех_ покупателей и для каждого покупателя вернёт все активные заказы. Заметьте, что это
...
`customer` - _обратная связь_ относительно `orders`, и сделаем это с помощью вызова метода
...
> Info: В Yii версии 1.1 была концепция с названием _scope_. Она больше не поддерживается в Yii версии 2.0, и вы
