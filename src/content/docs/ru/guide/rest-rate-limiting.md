...
Чтобы включить ограничение частоты запросов, _[[yii\web\User::identityClass|класс user identity]]_ должен реализовывать
...
- `loadAllowance()`: возвращает оставшееся количество разрешенных запросов и _UNIX-timestamp_ последней проверки
  ограничения.
- `saveAllowance()`: сохраняет оставшееся количество разрешенных запросов и текущий _UNIX-timestamp_.
