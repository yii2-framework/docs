...
- [[Yii::warning()]]: записывает _тревожное_ сообщение при возникновении неожиданного события.
- [[Yii::error()]]: записывает критическую ошибку, на которую нужно, как можно скорее, обратить внимание.

Эти методы позволяют записывать сообщения разных _уровней важности_ и _категорий_. Они имеют одинаковое описание функции `function ($message, $category = 'application')`, где `$message` передает сообщение для записи, а `$category` - категорию сообщения. В следующем примере будет записано _trace_ сообщение с категорией по умолчанию `application`:
...
Если свойство [[yii\log\Target::levels|levels]] не задано, цель логов будет обрабатывать сообщения с _любым_ уровнем важности.
...
Для использования профилирования производительности нужно определить участок кода для измерения и _обернуть_ его вызовами методов [[Yii::beginProfile()]] и [[Yii::endProfile()]]. Например,
