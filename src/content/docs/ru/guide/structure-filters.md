...
Фильтр может состоять из _пре-фильтра_ (фильтрующая логика применяется _перед_ действиями) и/или
_пост-фильтра_ (логика, применяемая _после_ действий).
...
По умолчанию фильтры, объявленные в классе контроллера, будут применяться ко _всем_ его действиям. Тем не менее, вы можете
...
В этом случае они применяются ко _всем_ действиям контроллеров, находящихся в этом модуле или приложении если не заданы
...
Ограничитель количества запросов в единицу времени _(RateLimiter)_ реализует алгоритм ограничения запросов, основанный на
...
Фильтр по типу запроса _(VerbFilter)_ проверяет, разрешено ли запросам HTTP выполнять затребованные ими действия.
