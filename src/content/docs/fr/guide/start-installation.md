...
> ```
>
> Notez que la version de développement de Yii ne doit pas être utilisée en production, vu qu'elle pourrait _casser_ votre code existant.
...
Les instructions d'installation ci-dessus montrent comment installer Yii, ce qui installe également une application Web de base qui fonctionne _out of the box_ (sans configuration supplémentaire).
...
![Successful Installation of Yii](/docs/guide/images/start-app-installed.png)
...
L'application installée selon les instructions ci-dessus devrait fonctionner _out of the box_ (sans configuration supplémentaire) avec le [serveur HTTP Apache](https://httpd.apache.org/) ou le [serveur HTTP Nginx](https://nginx.org/), sous Windows, Mac OX X, ou Linux avec PHP 5.4 ou plus récent. Yii 2.0 est aussi compatible avec
