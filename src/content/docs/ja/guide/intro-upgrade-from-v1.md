...
今や `$this` は 2.0 で新しく導入された概念である _ビュー_・オブジェクトを指します。
_ビュー_・オブジェクトは [[yii\web\View]] という型であり、MVC パターンのビューの部分を表すものです。
...
Yii 2.0 は、_アセット・バンドル_ と呼ばれる新しい概念を導入しました。これは、Yii 1.1 にあったスクリプト・パッケージの概念を置き換えるものです。
...
Yii 2.0 は [[yii\widgets\ActiveForm]] を使ってフォームを作成する際に使用する _フィールド_ の概念を導入しました。
