...
mówimy o _imiennym behaviorze_. W powyższym przykładzie znajdują się dwa imienne behaviory: `myBehavior2` i `myBehavior4`.
Jeśli behavior nie ma przypisanej nazwy, nazywamy go _anonimowym_.
...
Możesz uzyskać dostęp do _publicznej_ zmiennej lub [właściwości](concept-properties.md) zdefiniowanej przez getter i/lub setter
...
Możesz również wywołać _publiczną_ metodę behavioru w podobny sposób:
...
został załączony jako _pierwszy_, będzie obsługiwał wywołaną właściwość lub metodę.
...
Można również odłączyć _wszystkie_ behaviory jednocześnie:
