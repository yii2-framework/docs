...
Questo indirizzo assume che hai installato Yii in una directory di nome `basic`, direttamente nella _root_ del tuo webserver,
...
![Installazione di Yii completata con successo](/docs/guide/images/start-app-installed.png)
...
_document root_ del tuo webserver nella directory `basic/web`. Vorrai magari anche nascondere `index.php` dall'URL, come descritto
...
> Informazione: impostando `basic/web` come _document root_ impedisci agli utenti finali di accedere al codice e a file/informazioni
...
Usa questa configurazione nel file `httpd.conf` di Apache o nella definizione del tuo _VirtualHost_. Tieni presente che dovrai
