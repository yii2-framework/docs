...
Yii manages assets in the unit of _asset bundle_. An asset bundle is simply a collection of assets located
...
- [[yii\web\AssetBundle::jsOptions|jsOptions]]: specifies the options that will be passed to the
  [[yii\web\View::registerJsFile()]] method when it is called to register _every_ JavaScript file in this bundle.
- [[yii\web\AssetBundle::cssOptions|cssOptions]]: specifies the options that will be passed to the
  [[yii\web\View::registerCssFile()]] method when it is called to register _every_ CSS file in this bundle.
...
- source assets: the asset files are located together with PHP source code which cannot be directly accessed via Web.
  ...
  published assets. This process is called _asset publishing_ which will be described in detail shortly.
...
[[yii\bootstrap\BootstrapAsset]], which means the CSS and JavaScript files in `AppAsset` will be included _after_
...
> Note: The options you set in a bundle class apply to _every_ CSS/JavaScript file in the bundle. If you want to
...
You can also disable _all_ asset bundles by setting [[yii\web\AssetManager::bundles]] as `false`.
...
fix the problem by customizing each bundle, an easier way is to use the _asset map_ feature to map incorrect assets
...
to a Web directory when the bundle is being registered with a view. This process is called _asset publishing_, and is done
