...
Os models representam dados de negócio por meio de _atributos_. Cada atributo é
...
Um model (modelo) pode ser usado em diferentes _cenários_. Por exemplo, um model
...
os valores que correspondem aos _active attributes_ (atributo ativo). Um atributo
...
validados para garantir que satisfazem as regras (_regras de validação_, também
conhecidos como _regras de negócio_). Por exemplo, considerando um model (modelo)
...
cenários. Uma regra é chamada de _active rule_ (regra ativa), se ela puder ser
...
A atribuição em massa só se aplica aos chamados _safe attributes_ (atributos seguros),
...
quais itens de dados, chamados de _fields_ (campos), devem ser mostrados no array
