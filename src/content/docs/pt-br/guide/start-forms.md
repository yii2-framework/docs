...
> Info: [[yii\base\Model]] é usado como pai das classes de models que _não_
...
[[yii\base\Model::hasErrors|hasErrors]] será definida como _true_ e você pode
...
Você verá uma página exibindo um formulário com dois campos. Na frente de cada campo, um _label_ indica quais dados devem ser inseridos. Se você clicar
...
![Formulário com Erros de Validação](/docs/guide/images/start-form-validation.png)
...
![Confirmação de Registro](/docs/guide/images/start-entry-confirmation.png)
...
quase mágica exibir um _label_ para cada campo e mostrar mensagens
...
Os _labels_ dos campos são gerados pelo método `field()`, usando os nomes
das propriedades do model.
Por exemplo, um _label_ chamado `Nome` será gerado para a propriedade `nome`.

Você pode personalizar um _label_ em uma view utilizando o seguinte código:
