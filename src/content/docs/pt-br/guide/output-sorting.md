...
O atributo `age` é um atributo _simples_ que corresponde ao atributo `age` da classe Active Record `Article`. É equivalente a seguinte declaração:
...
O atributo `name` é um atributo _composto_ definido por `first_name` e `last_name` de `Article`. Declara-se com a seguinte estrutura de array:
