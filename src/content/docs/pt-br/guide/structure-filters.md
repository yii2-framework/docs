...
aplicadas _antes_ que as ações) e/ou um pós-filtro (lógica aplicada _depois_
que as ações).
...
aplicados em _todas_ as ações deste controller (controlador). Você pode, no
...
Quando você faz isso, os filtros serão aplicados em _todos_ as ações do controller
