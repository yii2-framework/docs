...
Для того, щоб увімкнути обмеження частоти запитів, _[[yii\web\User::identityClass|клас user identity]]_ повинен реалізовувати
...
- `loadAllowance()`: повертає кількість дозволених запитів, що залишились, та мітку часу _Unix_ останньої перевірки обмеження.
- `saveAllowance()`: зберігає кількість дозволених запитів та поточну мітку часу _Unix_.
