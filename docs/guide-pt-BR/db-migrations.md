...
a migração é sobre a criação de uma nova tabela chamada _noticias_, você pode usar o nome `criar_tabela_noticias`
...
Ao invés de usar tipos físicos, ao criar uma tabela ou coluna, você deve usar _tipos abstratos_ para que
...
Algumas vezes pode ocorrer que você queira aplicar _algumas_ das migrações para um banco de dados, e outras para
