...
manipular a requisição. Todo este processo é chamado de _roteamento_.

O processo inverso do roteamento é chamada de _criação de URL_, onde é criado uma
...
_rota padrão_ será usada em seu lugar. A rota padrão é `site/index` é utilizada
...
Ele pode ser tanto uma rota _relativa_ quanto uma rota _absoluta_ que será
...
foram declaradas e só enxergará a _primeira_ regra que corresponda a URL da
...
> SEO (otimização para mecanismos de pesquisa, do _inglês search engine optimization_).
