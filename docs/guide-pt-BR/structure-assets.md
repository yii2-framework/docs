...
O Yii gerencia os assets na unidade de _asset bundle_. Um asset bundle é
...
- [[yii\web\AssetBundle::jsOptions|jsOptions]]: especifica as opções que serão
  ...
  para registrar _cada_ arquivo JavaScript neste bundle.
- [[yii\web\AssetBundle::cssOptions|cssOptions]]: especifica as opções que serão
  ...
  para registrar _cada_ arquivo CSS neste bundle.
...
- assets fonte: os arquivos de asset estão localizados juntos ao código fonte
  ...
  _publicação de asset_ que será descrito em detalhes ainda nesta seção.
...
_após_ a inclusão dos arquivos dos dois bundles dependentes.
...
> Observação: As opções definidas em uma classe bundle aplicam-se para _todos_
...
Você também pode desabilitar _todos_ os asset bundles definindo o
...
um modo mais simples usando o recurso de _mapeamento de asset_ para mapear todos
...
chamado de _publicação de asset_ e é feito automaticamente pelo
