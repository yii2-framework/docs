...
Por exemplo, o conteúdo de um _post_ sendo exibido depende de ele ter sido ou não modificado.
...
### _Cache Alternante_ (Toggling Caching) <span id="toggling-caching"></span>
...
## _Cache Aninhado_ (Nested Caching) <span id="nested-caching"></span>

Cache de fragmentos pode ser aninhado. Isto é, um fragmento em cache pode estar contido em outro fragmento que também está em cache. Por exemplo, os comentários estão sendo armazenados em um cache de fragmento inserido em conjunto com o conteúdo do _post_ em outro cache de fragmento. O código a seguir exibe como dois caches de fragmento podem ser aninhados.
...
pode conter código PHP que deve ser executado para cada requisição (ex., o código para registrar um _pacote de recursos estáticos_ (asset bundles)). Ambos os problemas podem ser resolvidos com a funcionalidade, então chamada de _Conteúdos Dinâmicos_.
