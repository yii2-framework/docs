...
Os controllers são compostos por unidades básicas chamadas de _ações_ que podem
...
Os usuários finais abordarão as ações por meio de _rotas_. Uma rota é uma string composta
...
Criar ações pode ser tão simples como a definição dos chamados _métodos de ação_
em uma classe controller. Um método de ação é um método _público_ cujo nome inicia
...
> Observação: Os nomes dos métodos de ações são _case-sensitive_. Se você tiver
...
lugares ou se deseja distribuir uma ação, deve considerar defini-la como uma _ação standalone_.
...
Para usar uma ação standalone, você deve _mapear as ações_ sobrescrevendo o método
...
standalone podem receber parâmetros, chamados _parâmetros da ação_.
