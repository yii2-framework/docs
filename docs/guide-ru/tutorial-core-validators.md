...
вы можете использовать _псевдонимы_, чтобы указать на использование этих валидаторов.
...
Этот валидатор проверяет, что второе значение является _boolean_.
...
- `captchaAction`: [маршрут](structure-controllers.md#routes), соответствующий
  [[yii\captcha\CaptchaAction|CAPTCHA action]], который рендерит изображение с _CAPTCHA_. По умолчанию - `'site/captcha'`.
- `skipOnEmpty`: может ли валидация быть пропущена, если _input_ пустой. По умолчанию - `false`,
  что означает, что _input_ обязателен.
...
Этот валидатор проверяет, что входящее значение является корректным _double_ числом. Он идентичен
...
- `normalize`: нормализировать ли проверяемый IP адрес без CIDR к IP адресу с наименьшим CIDR
  ...
  _ `10.0.1.5` будет приведен к `10.0.1.5/32`
  _ `2008:db0::1` будет приведен к `2008:db0::1/128`
