...
- Si vous redéfinissez le constructeur, spécifiez un paramètre `$config` en tant que _dernier_ paramètre du constructeur et passez le au constructeur du parent.
- Appelez toujours le constructeur du parent _à la fin_ de votre constructeur redéfini.
- Si vous redéfinissez la méthode [[yii\base\BaseObject::init()]], assurez-vous que vous appelez la méthode `init()` mise en œuvre par le parent _au début_ de votre méthodes `init()`.
